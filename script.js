document.addEventListener("DOMContentLoaded", function () {
  ...
  function addLineButton() {
    const lineButtonContainer = document.querySelector("#line-button");
    const stationButtonContainer = document.querySelector("#station-button");
    const image = document.querySelector("#images");
    for (const line in allLine) {
      // make the button
      const lineButton = document.createElement("div");
      lineButton.classList.add(line);
      lineButton.classList.add("line");
      lineButton.style.color = allLine[line].color;
      lineButton.style.setProperty("--color", allLine[line].color);
      lineButtonContainer.appendChild(lineButton);
      // make the text inside the button
      const text = document.createElement("span");
      text.innerText = allLine[line].text;
      lineButton.appendChild(text);
      ...
      lineButton.addEventListener("click", async function createStationMap() {
        ...
        for (const station of allLine[line].sta) {
          ...
          stationButton.classList.add("station");
          stationButton.innerText = station.name;
          stationButton.appendChild(pin);
          stationButtonContainer.appendChild(stationButton);
        }
      });
    }
  }
